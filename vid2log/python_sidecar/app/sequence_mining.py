...
Copied verbatim from backend/app/services/sequence_mining.py — it's pure
algorithm with no cloud dependency of any kind, so the offline build runs
the exact same mining code as the cloud backend and produces identical
...
def frequent_patterns(
    ...
    (easy to hit by accident — see analytics.py's rounding) give this an
